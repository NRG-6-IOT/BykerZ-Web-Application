  ...
  template: `
    <div class="bg-[#380800] p-6 text-white min-w-[50rem] rounded-2xl">
      ...
      <div class="flex justify-end gap-4">
        <button
          class="bg-white text-black font-bold py-2 px-4 rounded-2xl hover:bg-gray-300 transition-colors"
          (click)="onCancelClick()"
          [disabled]="expenseStore.loading()">
          Cancel
        </button>
        <button
          class="bg-[#FF6B35] text-white font-bold hover:bg-[#ff9169] py-2 px-4 rounded-2xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          (click)="onCreate()"
          [disabled]="expenseStore.loading() || items().length === 0 || !expenseName.trim()">
          Create Expense
        </button>
      </div>
    </div>
