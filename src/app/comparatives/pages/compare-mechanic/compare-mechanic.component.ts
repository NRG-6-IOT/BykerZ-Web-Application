...
export class CompareMechanicComponent implements OnInit {
  ...
  private loadAllModels(): void {
    ...
    this.vehicleService.getAllModels().subscribe({
      next: (models) => {
        this.availableVehicles = models.map((model, index) => this.mapModelToVehicle(model, index));
  ...
  private mapModelToVehicle(apiModel: any, index: number): Vehicle {
    return new Vehicle({
      id: apiModel.id,
      ownerId: 0, // No owner for model comparison
      mechanicId: 0,
      model: new Model({
        id: apiModel.id,
        name: apiModel.name,
        brand: apiModel.brand,
        modelYear: apiModel.modelYear,
        originCountry: apiModel.originCountry,
        producedAt: apiModel.producedAt,
        type: apiModel.type,
        displacement: apiModel.displacement,
        potency: apiModel.potency,
        engineType: apiModel.engineType,
        engineTorque: apiModel.engineTorque,
        weight: apiModel.weight,
        transmission: apiModel.transmission,
        brakes: apiModel.brakes,
        tank: apiModel.tank,
        seatHeight: apiModel.seatHeight,
        consumption: apiModel.consumption,
        price: apiModel.price?.toString() || '0',
        oilCapacity: apiModel.oilCapacity,
        connectivity: apiModel.connectivity,
        durability: apiModel.durability,
        octane: apiModel.octane
      }),
      year: apiModel.modelYear,
      plate: `MODEL-${apiModel.id}`, // Placeholder plate for models
    });
  }
  ...
  onLeftSelect(id: number) {
    ...
    if (selected) {
      ...
      // If rightVehicle matches new leftVehicle, pick a different one
